...
class CognitoAuthProvider(AuthenticationProvider):
        ...
        try:
            ...
            if mfa_code and self._mfa_tokens:
                if not self._complete_mfa_challenge(mfa_code):
                    return False
            elif self.cognito_client is not None:
    ...
    def authenticate_admin(
        self, username: str, password: str, aws_profile: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Authenticating {username} using ADMIN_USER_PASSWORD_AUTH flow..."
            )

            if self.boto_cognito_admin_client is None:
    ...
    def _init_boto_admin_client(self, aws_profile: Optional[str] = None) -> None:
        try:
            ...
            raise RuntimeError(
                f"Failed to initialize client: AWS error [{error_code}]: {error_msg}"
            )
        ...
        try:
            ...
            if "SOFTWARE_TOKEN" in challenge_name:
                self.cognito_client.respond_to_software_token_mfa_challenge(
                    code=mfa_code, mfa_tokens=self._mfa_tokens
                )
            else:
