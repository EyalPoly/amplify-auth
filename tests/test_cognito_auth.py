...
class TestStandardAuthentication:
    ...
    def test_successful_authentication(
        self, mock_jwt_decode, mock_cognito_class, cognito_provider, mock_id_token
    ):
    ...
    @patch("amplify_auth.cognito_auth.Cognito")
    @patch("amplify_auth.cognito_auth.jwt.decode")
    def test_authentication_not_in_admin_group(
        self, mock_jwt_decode, mock_cognito_class, cognito_provider
    ):
    ...
    def test_authentication_with_mfa_challenge(
        self, mock_input, mock_cognito_class, cognito_provider, mock_id_token
    ):
        ...
        with patch.object(
            cognito_provider, "_complete_mfa_challenge", return_value=True
        ):
            with patch.object(cognito_provider, "_check_user_in_admins_group"):
                result = cognito_provider.authenticate(
    ...
    @patch("amplify_auth.cognito_auth.Cognito")
    @patch("amplify_auth.cognito_auth.input")
    def test_authentication_with_password_change(
        self, mock_input, mock_cognito_class, cognito_provider, mock_id_token
    ):
    ...
    @patch("amplify_auth.cognito_auth.Cognito")
    @patch("amplify_auth.cognito_auth.input")
    def test_authentication_password_mismatch(
        self, mock_input, mock_cognito_class, cognito_provider
    ):
...
class TestAdminAuthentication:
    ...
    def test_successful_admin_authentication(
        self, mock_jwt_decode, mock_boto_client, cognito_provider, mock_id_token
    ):
    ...
    def test_admin_authentication_with_profile(
        self, mock_session_class, cognito_provider, mock_id_token
    ):
        ...
        with patch("amplify_auth.cognito_auth.jwt.decode") as mock_jwt:
            ...
            result = cognito_provider.authenticate_admin(
                "test@example.com", "password", aws_profile="my-profile"
            )
    ...
    @patch("amplify_auth.cognito_auth.boto3.client")
    def test_admin_authentication_no_credentials(
        self, mock_boto_client, cognito_provider
    ):
    ...
    @patch("amplify_auth.cognito_auth.boto3.Session")
    def test_admin_authentication_profile_not_found(
        self, mock_session_class, cognito_provider
    ):
        """Test admin authentication fails with invalid profile"""
        mock_session_class.side_effect = ProfileNotFound(profile="invalid")

        result = cognito_provider.authenticate_admin(
            "test@example.com", "password", aws_profile="invalid"
        )
        assert result is False
    ...
    @patch("amplify_auth.cognito_auth.boto3.client")
    def test_admin_authentication_client_error(
        self, mock_boto_client, cognito_provider
    ):
        """Test admin authentication handles AWS client errors"""
        error_response = {"Error": {"Code": "AccessDenied", "Message": "Access denied"}}
        mock_boto_client.side_effect = ClientError(
            error_response, "admin_initiate_auth"
        )
    ...
    def test_admin_authentication_no_result(self, mock_boto_client, cognito_provider):
        ...
        with patch.object(
            cognito_provider, "_check_for_mfa_challenges", return_value=False
        ):
            result = cognito_provider.authenticate_admin(
                "test@example.com", "password123"
            )
...
class TestMFAChallenge:
    ...
    def test_complete_mfa_challenge_error(self, cognito_provider):
        ...
        mock_cognito.respond_to_sms_mfa_challenge.side_effect = Exception(
            "Invalid code"
        )
...
class TestCheckForMFAChallenges:
    ...
    def test_mfa_setup_challenge(self, cognito_provider):
        ...
        result = cognito_provider._check_for_mfa_challenges(
